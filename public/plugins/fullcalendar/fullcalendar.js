...
var DragListener = Class.extend({
	...
	mousedown: function(ev) {
		if (isPrimaryMouseButton(ev)) {
			...
			// console.log(this.coordMap.getCell(ev.pageX, ev.pageY));



			console.log(this.getCell(ev).start._d); //this line is to get value of the day that want to get data
			$date = this.getCell(ev).start._d;
			$token = $("#token input").val();
			// alert(this.getCell(ev).start._d);
			$.post('/data/save',{'date':$date,'_token':$token},function(response){
